...
const reverseModels = function (sequelize, models) {
  ...
  for (let model in models) {
    ...
    if (typeof models[model].options.charset !== "undefined") {
      ...
      // tables[models[model].tableName].charset = 'utf8'
    }
...
const parseDifference = function (previousState, currentState) {
  ...
  //log(JSON.stringify(difference, null, 4));
          ...
          if (df.path[1] === "schema") {
            ...
            if (df.path.length > 3) {
              if (df.path[1] === "schema") {
                ...
                const hashOfAction = hash(action);
                //[CC 2022-08-04] when we have a change column change the script generates an action that describes
                // completely the column, so if we have multiple changes to the definition of a single column we
                // don't have to manage them one by one, a single action is enough
                if (actionsHash.indexOf(hashOfAction) === -1) {
                  actions.push(action);
                  actionsHash.push(hashOfAction);
                }
                break;
              }
            }
...
const writeMigration = function (
  ...
  name = name.replace(" ", "_");
  //[CC 2022-08-04] adding left pad on version and adding automatically date and time
  ...
  const shortFilename =
    _.padStart(revision, 6, "0") +
    `_${date}_${time}_` +
    (name != "" ? `_${name}` : "") +
    ".js";
  const filename = path.join(migrationsDir, shortFilename);
  ...
  const upFunction = useContextObject ? "async function({ context: queryInterface })" : "async function(queryInterface, Sequelize))";
  const migrationType = useContextObject ? "import('sequelize-auto-migrations-v2').MigrationV2" : "import('sequelize-auto-migrations-v2').Migration";

  const sequelizeImport = useContextObject ? "import Sequelize from 'sequelize';" : "const Sequelize = require('sequelize');";
